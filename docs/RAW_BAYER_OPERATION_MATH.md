...
> ⚠️ **历史文档 (2026-08-17)**：固件已移除 `'T'` 命令和双窗口机制。
> 当前固件输出单一 320×240 窗口帧流（CAM2），无需上下切换、缝合或 `stitch_halves`。
> 本节以下内容为历史参考，记录了双窗口设计的原理和缺陷修复过程。
...
> ⚠️ **历史文档 (2026-08-17)**：本节基于双窗口帧流（`upper_000.raw`/`lower_000.raw` + `stitch_halves`），
> 描述的是固件移除 `'T'` 命令前的旧架构。当前固件输出单一 320×240 窗口帧流，
> 无需上下切换和缝合。数学原理仍适用，但数据流和 CLI 参数已改变。
